let S,g,_,q=Promise.resolve();function b(e){return e!==null&&typeof e=="object"&&!Array.isArray(e)}function s(e,t,n,i){const r=new Error(t);return r.name=e,n&&(r.code=n),i!==void 0&&(r.details=i),r}function M(e){const t=e instanceof Error?e:new Error(String(e)),n={name:typeof t.name=="string"?t.name:"Error",message:typeof t.message=="string"?t.message:String(t)};if((typeof t.code=="string"||typeof t.code=="number")&&(n.code=t.code),typeof t.resultCode=="number"&&(n.resultCode=t.resultCode),t.details!==void 0)try{JSON.stringify(t.details),n.details=t.details}catch{n.details=String(t.details)}return typeof t.stack=="string"&&(n.stack=t.stack),n}function T(e,t){self.postMessage({id:e??null,type:"response",ok:!0,result:t})}function R(e,t){self.postMessage({id:e??null,type:"response",ok:!1,error:M(t)})}async function O(){return S||(S=import("./index-DlV0y7TX.js").then(async e=>{const t=e.default??e.sqlite3InitModule;if(typeof t!="function")throw s("SQLiteLoadError","O módulo @sqlite.org/sqlite-wasm não exportou sqlite3InitModule.","SQLITE_MODULE_INVALID");return t()})),S}function h(){if(!_)throw s("DatabaseNotInitializedError","O banco SQLite ainda não foi inicializado.","DB_NOT_INITIALIZED");return _}function f(e,t,n){const i=[],r={sql:t,rowMode:"object",resultRows:i};return n!==void 0&&(r.bind=n),e.exec(r),i}function u(e){return typeof e=="bigint"?Number(e):e}function D(e){return e instanceof ArrayBuffer||typeof SharedArrayBuffer<"u"&&e instanceof SharedArrayBuffer}function x(e){return D(e)?new Uint8Array(e):ArrayBuffer.isView(e)?new Uint8Array(e.buffer,e.byteOffset,e.byteLength):null}function v(e){const t=e?.payload,n=[t,t?.buffer,t?.arrayBuffer,e?.buffer,e?.arrayBuffer];for(const i of n){const r=x(i);if(r)return r}return null}function L(){const e=_;_=void 0,e&&e.close()}async function C(e){const t=v(e);if(!t||t.byteLength===0)throw s("InvalidArgumentError","init exige um ArrayBuffer não vazio contendo um banco SQLite.","INVALID_DATABASE_BUFFER");g=await O(),L();const n=new g.oo1.DB;let i,r=!1;try{i=g.wasm.allocFromTypedArray(t);const l=g.capi.sqlite3_deserialize(n.pointer,"main",i,t.byteLength,t.byteLength,0);n.checkRc(l),r=!0,n.onclose={after:()=>{i&&(g.wasm.dealloc(i),i=void 0)}};const E=f(n,`SELECT name
      ...
      JOIN (
        ...
        OR EXISTS (SELECT 1 FROM alternatives AS alternative_image_filter
                   WHERE alternative_image_filter.question_id = q.id
                     AND alternative_image_filter.file_url IS NOT NULL))`),e.excludeIds.length>0){const l=e.excludeIds.map((E,o)=>{const a=`$exclude_${o}`;return t[a]=E,a});n.push(`q.id NOT IN (${l.join(", ")})`)}const r=n.length>0?`WHERE ${n.join(`
          AND `)}`:"";return{bind:t,join:i,where:r}}function B(e){return{id:Number(u(e.id)),year:Number(u(e.year)),number:Number(u(e.number)),language:e.language??null,title:e.title,discipline:e.discipline??null,hasImages:!!Number(u(e.has_images)),snippet:e.snippet??null}}function Q(e){const t=h(),n=U(e),i=F(n),r=f(t,`SELECT count(*) AS total
       FROM questions AS q
       ${i.join}
       ${i.where}`,i.bind),l=Number(u(r[0]?.total)??0),E={...i.bind,$limit:n.limit,$offset:n.offset},o=n.ftsQuery?`(SELECT snippet(search_index, 3, '', '', '…', 18)
         FROM search_index
    ...
    SELECT paged.id,
      ...
      FROM paged`,E);return{total:l,page:n.page,pageSize:n.pageSize,results:a.map(B)}}function G(e){if(typeof e=="number"||typeof e=="string")return{id:e};if(!b(e))throw s("InvalidArgumentError","getQuestion exige um ID ou uma chave de questão.","INVALID_ARGUMENT");return e}function H(e){const t=h(),n=G(e),i=m(n.id??n.questionId,"id",{min:1}),r=m(n.year,"year"),l=m(n.number,"number",{min:1}),E=y(n.language,"language");if(i===void 0&&(r===void 0||l===void 0))throw s("InvalidArgumentError","getQuestion exige id/questionId ou o conjunto year + number.","INVALID_ARGUMENT");let o;if(i!==void 0)o=f(t,`SELECT id, year, number, language, title, discipline, context,
              alternatives_introduction, correct_alternative, source_path
         FROM questions
        WHERE id = $id`,{$id:i});else{const c={$year:r,$number:l},N=["year = $year","number = $number"];if(E!==void 0&&(N.push("language = $language"),c.$language=E),o=f(t,`SELECT id, year, number, language, title, discipline, context,
              alternatives_introduction, correct_alternative, source_path
         FROM questions
        WHERE ${N.join(" AND ")}
        LIMIT 2`,c),o.length>1)throw s("AmbiguousQuestionError","Mais de uma questão corresponde a year + number; informe language ou id.","AMBIGUOUS_QUESTION")}const a=o[0];if(!a)throw s("QuestionNotFoundError","Questão não encontrada.","QUESTION_NOT_FOUND");const d=Number(u(a.id)),I=f(t,`SELECT id, letter, text, file_url, is_correct
       FROM alternatives
      WHERE question_id = $questionId
      ORDER BY letter COLLATE NOCASE, id`,{$questionId:d}).map(c=>({id:Number(u(c.id)),letter:c.letter,text:c.text??null,file:c.file_url??null,fileUrl:c.file_url??null,isCorrect:!!Number(u(c.is_correct))})),p=f(t,`SELECT url
       FROM question_files
      WHERE question_id = $questionId
      ORDER BY position, id`,{$questionId:d}).map(c=>c.url);return{id:d,year:Number(u(a.year)),number:Number(u(a.number)),language:a.language??null,title:a.title,discipline:a.discipline??null,context:a.context??null,alternativesIntroduction:a.alternatives_introduction??null,correctAlternative:a.correct_alternative??null,sourcePath:a.source_path,alternatives:I,files:p,urls:p}}async function X(e){const t=e?.id??null;if(!b(e)||typeof e.type!="string"){R(t,s("InvalidMessageError","Mensagem de worker inválida.","INVALID_MESSAGE"));return}try{let n;switch(e.type){case"init":n=await C(e);break;case"getFilters":n=$(e.payload);break;case"searchQuestions":n=Q(e.payload);break;case"getQuestion":n=H(e.payload);break;case"close":L(),n={closed:!0};break;default:throw s("UnknownMessageError",`Operação desconhecida: ${e.type}.`,"UNKNOWN_MESSAGE")}T(t,n)}catch(n){R(t,n)}}typeof self<"u"&&self.addEventListener("message",e=>{q=q.then(()=>X(e.data))});
